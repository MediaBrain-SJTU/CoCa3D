...
class UncertaintyVoxelPostprocessor(BasePostprocessor):
    ...
    def post_process_stage1(self, stage1_output_dict, anchor_box):
        ...
        sm = stage1_output_dict['sm']

        dm = None
        if 'dm' in stage1_output_dict.keys():
            dm = stage1_output_dict['dm']

        # the final bounding box list
        uncertainty_dim = sm.shape[1] // psm.shape[1]
        ...
        # sm
        sm = sm.permute(0,2,3,1).contiguous() #[N, H, W, anchor_num * 2]


        # convert regression map back to bounding box
        batch_box3d = self.delta_to_boxes3d(reg, anchor_box)  # (N, W*L*2, 7)
        ...
        uncertainty = torch.masked_select(sm.view(-1,uncertainty_dim), mask_sm[0]).view(-1,uncertainty_dim) # [N*H*W*#anchor_num, 2] -> [num_select, 2]
        scores = torch.masked_select(prob.view(-1), mask[0])
        if dm is not None and len(boxes3d) != 0:
    ...
    def post_process(self, data_dict, output_dict, return_uncertainty=False):
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            uncertainty_dim = output_dict[cav_id]['sm'].shape[1] // output_dict[cav_id]['psm'].shape[1]
            ...
            # uncertainty map
            sm = output_dict[cav_id]['sm'].permute(0, 2, 3, 1).contiguous()
            sm = sm.view(sm.shape[0], -1, uncertainty_dim) # [N, H*W*#anchor_num, 2]
            ...
            uncertainty = torch.masked_select(sm[0], mask_sm[0]).view(-1, uncertainty_dim)
