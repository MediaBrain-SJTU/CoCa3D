...
from opencood.models.sub_modules.dense_head import Head
from opencood.models.sub_modules.voxel_roi_pooling import VoxelRoIPooling
from opencood.models.sub_modules.voxel_rcnn_head import RoIHead
from opencood.models.sub_modules.proposal_fuse import ProposalFuser
from opencood.data_utils.post_processor.fvoxelrcnn_postprocessor import \
    FVoxelRCNNPostprocessor
...
class FVoxelRCNN(nn.Module):
    def __init__(self, args):
        ...
        self.head = Head(args['head'])
        self.post_processor = FVoxelRCNNPostprocessor(args['post_processer'],
                                                   train=self.training)
        # self.vsa = VoxelSetAbstraction(args['vsa'], args['voxel_size'],
        #                                args['lidar_range'],
        #                                num_bev_features=128,
        #                                num_rawpoint_features=3)
        self.voxel_roi_pooling = VoxelRoIPooling(self.spconv_block.backbone_channels, 
                                                 args['voxel_roi_pooling'],
                                                 args['voxel_size'],
                                                 args['lidar_range'])
        self.fuser = ProposalFuser(args['proposal_fuser'], args['lidar_range'])
