...
from opencood.utils.box_utils import corner_to_center_torch, boxes_to_corners_3d, project_box3d, project_points_by_matrix_torch
...
class ProposalFuser(nn.Module):
    """Cluster boxes and fuse the grid feature"""

    def __init__(self, cfg, pc_range):
        super(ProposalFuser, self).__init__()
        self.pc_range = pc_range

    @torch.no_grad()
    def forward(self, data_dict):
        clusters, scores, pooled_features = self.clustering(data_dict)
        data_dict['boxes_fused'], data_dict[
            'scores_fused'], data_dict['pooled_features'] = self.cluster_fusion(clusters, scores, pooled_features)
        self.merge_pooled_features(data_dict)
        return data_dict
        ...
        for i, l in enumerate(record_len):
            ...
            if data_dict['proj_first'] is False:
                ...
                for agent_id in range(1, l):
                    ...
                    cur_corners = boxes_to_corners_3d(cur_boxes, order='lwh')
                    cur_corners_ego = project_box3d(cur_corners, tfm)
                    cur_boxes_ego = corner_to_center_torch(cur_corners_ego, order='lwh')
            ...
            cur_pooled_feature_list = data_dict['pooled_features'][sum(record_len[:i]:sum(record_len)+l)]
    ...
    def merge_pooled_features(self, data_dict):
        # merge keypoints
        kpts_feat_out = []
        kpts_coor_out = []
        kpts_coor_out_ego = []
