...
import numpy as np
from opencood.loss.point_pillar_dir_loss import PointPillarDirLoss, weighted_smooth_l1_loss


class FpvrcnnLoss(nn.Module):
    def __init__(self, args):
        super(FpvrcnnLoss, self).__init__()
        self.stage1_loss = PointPillarDirLoss(args['stage1'])
    ...
    def forward(self, output_dict, label_dict):
        ...
        target_dict : dict
        """
        stage1_loss = self.stage1_loss(output_dict, label_dict)

        # only update ciassd if no bbox is detected in the first stage
        if 'fvoxelrcnn_out' not in output_dict:
            self.loss_dict = {
                'loss': stage1_loss,
            }
            return stage1_loss
